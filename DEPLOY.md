# Hosting the BGA newsletter admin panel
...
2. **New Project → Deploy from GitHub repo** → pick `bga-newsletter`.
...
4. After the first deploy, set `APP_BASE_URL` to the service URL
   (e.g. `https://bga-newsletter.onrender.com`), or to `https://newsletter.bga.ge`
   once the custom domain below is pointed at it.
...
```
docker build -t bga-newsletter .
docker run -d --name bga-newsletter -p 3000:3000 \
  -v bga-newsletter-data:/data --env-file .env --restart unless-stopped bga-newsletter
```
...
- **Regenerate the Mailchimp API key** (Mailchimp → Account → Extras → API
  ...
  in chat or email.
- **Custom domain `newsletter.bga.ge`:** add it in the host's dashboard,
  create the CNAME record it shows you in the `bga.ge` DNS zone, wait for the
  certificate to be issued, then set `APP_BASE_URL` to
  `https://newsletter.bga.ge`. Do this before inviting staff - the login
  links in invitation and reminder emails are built from `APP_BASE_URL`.
