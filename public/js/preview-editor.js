/* BGA Newsletter - live preview editor.
...
(function () {
  ...
  /* Editor chrome colours. The editor is injected into the newsletter preview
     document, which has no access to the admin stylesheet, so it carries its
     own copy of the palette - keep these four in sync with src/brand.js
     (`npm test` checks that they match). */
  var C = { brand: '#12306b', brandDeep: '#0a1c45', accent: '#d8232a', accentDeep: '#a3151b', ink: '#3a4258' };
  function rgb(hex) {
    return [1, 3, 5].map(function (i) { return parseInt(hex.substr(i, 2), 16); }).join(',');
  }
  var ACCENT_RGB = rgb(C.accent);

  var style = document.createElement('style');
  style.textContent =
    '[data-edit]{cursor:text; transition:box-shadow .12s; border-radius:4px;}' +
    '[data-edit]:hover{box-shadow:0 0 0 2px rgba(' + ACCENT_RGB + ',.85); background:rgba(' + ACCENT_RGB + ',.08);}' +
    '[data-edit].re-editing{box-shadow:0 0 0 2px ' + C.brand + '; background:#fff; color:' + C.ink + '; outline:none; min-width:40px; display:inline-block;}' +
    ...
    '.re-photo-wrap:hover img{outline:2px solid rgba(' + ACCENT_RGB + ',.9);}' +
    '.re-btn{font-family:FiraGO,Arial,sans-serif; font-size:11px; font-weight:600; border:none; border-radius:6px; padding:4px 9px; cursor:pointer; background:' + C.brandDeep + '; color:#fff;}' +
    '.re-btn.re-danger{background:#B23A32;}' +
    '.re-add{display:inline-block; margin:10px 0 4px 0; border:2px dashed #c7ccd6; background:#f7f8fa; color:#6b7280; font-family:FiraGO,Arial,sans-serif; font-size:12px; font-weight:600; border-radius:8px; padding:8px 14px; cursor:pointer;}' +
    '.re-add:hover{border-color:' + C.accent + '; color:' + C.accentDeep + ';}' +
    '.re-bar{position:sticky; top:0; z-index:60; background:' + C.brandDeep + '; color:#fff; font-family:FiraGO,Arial,sans-serif; font-size:12.5px; padding:9px 16px; display:flex; align-items:center; gap:10px; box-shadow:0 2px 10px rgba(0,0,0,.25);}' +
    '.re-bar b{color:' + C.accent + '; font-weight:600; letter-spacing:1px;}' +
    ...
    '[data-masthead]:hover{outline:2px solid rgba(' + ACCENT_RGB + ',.9); outline-offset:-2px;}' +
    ...
    '.re-drop{outline:3px dashed ' + C.accent + ' !important; outline-offset:3px; border-radius:10px;}' +
