...
(function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!files.length) return log('Choose the photo files first.', getComputedStyle(document.documentElement).getPropertyValue('--muted').trim() || '#6b7280');
    ...
    (function next() {
      if (i >= files.length) {
        ...
        log('Done: ' + matched + ' matched and saved, ' + skipped + ' skipped (of ' + files.length + ').', getComputedStyle(document.documentElement).getPropertyValue('--brand').trim() || '#12306b');
        return;
      }
      var file = files[i++];
