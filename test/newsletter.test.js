...
const baseData = {
  ...
  footerNote: 'Nino Beridze - Principal\nSimon Rooney - Head of Primary',
  calendarUrl: 'https://bga.ge/calendar',
};
...
test('renderNewsletter includes all template sections', () => {
  ...
  assert.match(html, /bga\.ge\/calendar/);
});
...
test('footer staff directory renders names and titles', () => {
  const html = renderNewsletter(baseData);
  assert.match(html, /Nino Beridze/);
...
test('masthead background image renders email-safe with the brand-colour fallback', () => {
  ...
  assert.ok(withBg.includes(`background-color:${palette.brandDeep}`), 'brand colour shows while the image loads');
...
test('article text: bare URLs and [text](url) become safe styled links', () => {
  const html = textToHtml('See [our site](https://bga.ge/news) and https://example.com/page?a=1&b=2. Also [x](javascript:alert(1))');
  assert.match(html, /<a href="https:\/\/bga\.ge\/news"[^>]*>our site<\/a>/);
