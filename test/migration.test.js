...
test('booting migrates a v1 database: CHECK constraints gone, new roles/areas insert', () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bga-migrate-'));
  ...
  old.exec(`
    ...
    INSERT INTO users (email, name, password_hash, role) VALUES ('old.teacher@bga.ge', 'Old Teacher', 'hash', 'primary');
  ...
  const teacher = db.prepare("SELECT * FROM users WHERE email = 'old.teacher@bga.ge'").get();
  ...
  db.prepare("INSERT INTO users (email, name, password_hash, role, section) VALUES ('new.staff@bga.ge', 'New Staff', '', 'staff', NULL)").run();
  db.prepare("INSERT INTO users (email, name, password_hash, role, section) VALUES ('new.slt@bga.ge', 'New SLT', '', 'slt', 'sixth_form')").run();
...
test('stripCheckClauses removes nested-paren CHECKs and nothing else', () => {
  process.env.DATA_DIR = process.env.DATA_DIR || fs.mkdtempSync(path.join(os.tmpdir(), 'bga-strip-'));
