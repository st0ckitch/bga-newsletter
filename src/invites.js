...
async function sendStaffInvites(users = neverInvited()) {
  ...
  if (!isPublicUrl(baseUrl)) {
    return {
      sent: false,
      reason: `The app's public address is still ${baseUrl}, so the login links would not open for anyone. Set the APP_BASE_URL variable to your public URL (e.g. ${identity.publicUrlExample}) and try again.`,
    };
  }
