...
const { identity, palette } = require('./brand');
...
// Each house tile in the newsletter is a solid block of the house's own
// colour. Rows from before the column existed start in the school's brand
// colour and are recoloured from the Houses page.
...
db.prepare('UPDATE houses SET color = ? WHERE color IS NULL').run(palette.brand);
...
db.exec(`UPDATE settings SET value = '${identity.replyTo}' WHERE key = 'reply_to' AND value = ''`);
...
const SETTING_DEFAULTS = {
  ...
  editor_email: '',
  newsletter_name: identity.newsletterName,
  school_name: identity.schoolName,
  from_name: identity.schoolName,
